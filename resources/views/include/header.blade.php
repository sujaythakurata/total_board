<div class="header-outer">
    <header class="comman-header">
        <div class="container-fluid">
            <div class="row">
                <div class="col comman-header-col-left">
                    <div class="logoname-wrap">
                        <a href="/dashboard" target="_self">
                            <img src='{{asset("images/logo.png")}}' alt="WebApp" />
                        </a>
                    </div>
                </div>
                <div class="col top-nav-xs-wrap st comman-header-col-right">
                        <ul class="nav-top-right list-inline left-side-nav-wrap">
                            <li class="list-inline-item">
                                <div class="header-input-group input-group input-group-sm">
                                    <div class="input-group-prepend">
                                        <h4 style="color:#17a2b8" id="name">Production Line-910</h4>
                                    </div>
                                </div>
                            </li>_____
                                <li class="list-inline-item">
                                    <div class="header-input-group input-group input-group-sm">
                                        <div class="input-group-prepend">
                                            <h5 style="color:#fff">
                                                Batch No:
                                                <span style="color: #17a2b8" id="batchno">
                                                </span>
                                                Product:
                                                <span style="color: #17a2b8" id="pdname">
                                                </span>
                                            </h5>
                                        </div>
                                    </div>
                                </li>                              
                        </ul>
                    <ul class="nav-top-right list-inline right-side-nav-wrap">
                        <li class="nav-item list-inline-item nav-notification">
                            <a class="nav-link" href="/settings" data-toggle="tooltip" title="Setting">
                                <i class="fa fa-wrench" aria-hidden="true"></i>
                            </a>
                        </li>
                        <li class="nav-item list-inline-item nav-notification">
                            <a class="nav-link" href="{{url('/logout')}}" data-toggle="tooltip" title="Log Out">
                                <i class="fa fa-sign-out" aria-hidden="true"></i>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </header>
</div>